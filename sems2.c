...
int main() {
        ...
        char pathname[] = "for_ipc.ipc";//Имя файла, с помощью которого шенерируется ipc-ключ
        key_t key; //Переменная для хранения ipc-ключа
        struct sembuf mybuf; //Структура данных, задающая операции над определённым семафором
        // Процесс генерации ipc-ключа
        if((key = ftok(pathname, 0)) < 0) {
                printf("Не удалось сгенерировать ключ ipc\n");
                exit(-1);
        }
        //Процесс получения доступа к массиву семафоров для конкпетного ipc-ключа
